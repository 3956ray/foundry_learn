...
task('lz:oapp:send', 'Sends a string cross‐chain using OApp contracts')
    ...
    .setAction(async (args: { dstEid: number; string: string; options?: string }, hre: HardhatRuntimeEnvironment) => {
        ...
        // Select contract name by network
        const CONTRACT_NAME_BY_NETWORK: Record<string, string> = {
            'arbitrum-sepolia': 'arbOApp',
            'base-sepolia': 'baseOApp',
        }

        const selectedName = CONTRACT_NAME_BY_NETWORK[hre.network.name]
        if (!selectedName) {
            DebugLogger.printErrorAndFixSuggestion(
                KnownErrors.ERROR_GETTING_DEPLOYMENT,
                `Unsupported network for lz:oapp:send: ${hre.network.name}`
            )
            throw new Error(`Unsupported network: ${hre.network.name}`)
        }

        // Get the deployed OApp contract
        let oappContract
        let contractAddress: string
        try {
            const deployment = await hre.deployments.get(selectedName)
            contractAddress = deployment.address
            oappContract = await hre.ethers.getContractAt(selectedName, contractAddress, signer)
            logger.info(`${selectedName} contract found at: ${contractAddress}`)
        } catch (error) {
            DebugLogger.printErrorAndFixSuggestion(
                KnownErrors.ERROR_GETTING_DEPLOYMENT,
                `Failed to get ${selectedName} deployment on network: ${hre.network.name}`
            )
            throw error
        }
        ...
        let messagingFee
        try {
            messagingFee = await oappContract.quoteSendString(
                args.dstEid,
                args.string,
                options,
        ...
        try {
            tx = await oappContract.sendString(args.dstEid, args.string, options, {
